...
const Keyword = (props) => {
    ...
    const remove = (index) => {
        ...
        setIKeyword([...keyword])
        props.change(props.field, keyword)
    }
            ...
            <div className="tagInputContainer" style={{position: "relative"}}>
                {
                  keyword.map((item, index) => (
                    ...
                    <span>{item}</span>
                    <a onClick={()=>remove(index)} className="remove">×</a></div>
                  ))
                }
                <Input ref={keywordInput} prefix={<PlusOutlined style={{color: "#ccc"}} />} placeholder="请输入关键词" allowClear  onBlur={handelBlur} style={{border: 0, width: 150}} />
              ...
              isSearch ?
                <div className="tagListContainer">
                    <div className="tagList">
                      {
                        list.map((item, index) => (
                        <div onClick={select} className="tag-box tagBox">{item.name}</div>
                        ))
                      }                      
                    </div>
                </div>
